...
const Header = () => {
	...
	// Fetch the number of stargazers of the repository (musicForCode)
	useEffect(() => {
		const fetchStars = async () => {
			const response = await fetch("https://api.github.com/users/scmishra-cse/repos");
			const repos = await response.json();

			const repo = repos.filter(repo => repo.name === "musicforcode");
	...
	return (
		<header className="w-full bg-[#fafafa] flex flex-col sm:flex-row items-center justify-between shadow-sm p-2 dark:bg-[#1a1a1a]">
			...
			{/* Logo */}
			<div>
				<h1 className="font-bold text-md md:text-2xl lg:text-2xl text-gray-700 dark:text-gray-300">MusicForCode 🎵</h1>
			</div>

			<div className="flex items-center space-x-2">

				{/* Twitter Card */}
				<a href="https://www.twitter.com/shubh6200">
					<div className="flex items-center border border-gray-300 rounded-md px-2 py-1 bg-white transition-all duration-250 hover:bg-gray-100 dark:bg-black dark:border-gray-500">
						...
						{/* Shubham Chandra Mishra's Photo */}
						<div className="px-2">
							<img
								src="/shubham.jpg"
								alt="Shubham Chandra Mishra"
								width={40}
								height={40}
						...
						<div className="flex flex-col">
							<h2 className="font-bold text-gray-700 dark:text-gray-400">By Shubham Chandra Mishra</h2>
